...
| Replace photo | Hover any photo → click "Replace photo" → file picker → auto-compressed to 2048px wide. **Or drag an image file onto the photo** to swap it directly. |
| Insert photo via drag-and-drop | Drag an image file from your desktop onto empty space inside the doc → a new photo block appears at the closest sortable container's end, image auto-compressed to 2048px |
...
/* When a photo wrapper has been free-resized (browser's resize:both handle
   adds inline width/height to the element), force the inner <img> to fill
   the wrapper. Without this, the template's fixed-height img stays put
   while the wrapper grows around it, leaving the user staring at a strip
   of empty space below the image instead of a taller image. */
.dae-photo-wrap.dae-sortable-block[style*="height"],
.dae-photo-wrap.dae-sortable-block[style*="width"] {
  display: block;
}
.dae-photo-wrap.dae-sortable-block[style*="height"] img[data-editable-photo],
.dae-photo-wrap.dae-sortable-block[style*="width"] img[data-editable-photo] {
  width: 100% !important;
...
/* Drag-and-drop image feedback — body class toggled by global drag listeners.
   The ::before is the floating "Drop image..." banner; the outlines call out
   the existing photos as drop targets. */
body.dae-dragging::before {
  content: "Drop an image to replace a photo or add a new photo block";
  position: fixed; top: 24px; left: 50%; transform: translateX(-50%); z-index: 1001;
  background: var(--dae-accent, #0066cc); color: white;
...
body.dae-dragging [data-editable-photo] {
  outline: 3px dashed var(--dae-accent, #0066cc) !important;
  outline-offset: 4px;
}
body.dae-dragging [data-pdf-root] {
  outline: 2px dashed var(--dae-tint-1, #cce0ff);
  outline-offset: 8px;
  border-radius: 4px;
}
...
/* Block controls sit INSIDE the block at top-right (not floating above with
   top: -14px) because the free-resize feature forces overflow: auto on
   every block, which clips anything positioned outside the content rect. */
.dae-block-controls { position: absolute; top: 4px; right: 4px; display: flex; gap: 4px; pointer-events: none; opacity: 0; transition: opacity 0.15s; z-index: 5; background: var(--dae-bg, #ffffff); padding: 2px; border-radius: 6px; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.18); }
...
(() => {
  ...
  function enterRepositionMode(img) {
    ...
    // Click outside the photo also commits + exits. Without this, the user
    // is stuck — the overlay (with the Replace button) is hidden until they
    // discover the Esc shortcut. Attached on a microtask so the initial
    // click that entered reposition mode doesn't also fire this handler.
    const onDocClick = (e) => {
    ...
    toast('Drag to reposition · Esc or click outside to commit');
  }
  ...
  function exitRepositionMode(img) {
    ...
    if (img._daeRepositionCleanup) {
      img._daeRepositionCleanup();
      delete img._daeRepositionCleanup;
    }
  ...
  // Drop an image file onto any photo → replace it. Drop into empty space
  // inside the doc → insert a new photo block. Same compression pipeline
  // as the file-picker flow (auto-resize to 2048px). Only active in edit
  // mode. Files-only — non-image drops fall through to browser default
  // (Sortable's own drag events use HTML5 DnD but with no `Files` in
  // dataTransfer.types, so the two coexist cleanly).
  //
  // `_dragDepth` counter handles bubbling dragenter/dragleave events so
  // the body class only clears when the cursor leaves the entire window,
  // not just when crossing nested element boundaries.
  ...
  document.addEventListener('dragenter', (e) => {
    if (!editMode || !_isFileDrag(e)) return;
    _dragDepth++;
    document.body.classList.add('dae-dragging');
  });
  document.addEventListener('dragleave', (e) => {
    ...
    if (_dragDepth <= 0) {
      _dragDepth = 0;
      document.body.classList.remove('dae-dragging');
    }
  });
  ...
  document.addEventListener('drop', async (e) => {
    ...
    // 1. Drop on a photo → replace it.
    const photoTarget = e.target.closest && e.target.closest('[data-editable-photo]');
    if (photoTarget) {
      pushUndo('photo-replace', { target: photoTarget, prevSrc: photoTarget.src });
      toast('Compressing dropped photo…');
      try {
        photoTarget.src = await compressImage(imageFile);
    ...
    // 2. Drop into empty space → insert a new photo block at the end of
    //    the closest sortable container under the drop. If the drop wasn't
    //    inside the doc at all, fall back to [data-pdf-root].
    const root = findPdfRoot();
    let container = (e.target.closest && e.target.closest(SORTABLE_CONTAINER)) || root;
    if (!container || (root && !root.contains(container) && container !== root)) {
      toast('Drop inside the document to add a photo.');
      return;
    }
    const tpl = $('#tpl-photo');
    if (!tpl) return;
    ...
    if (!newBlock) return;
    const addBtn = container.querySelector(':scope > .dae-add-block-btn');
    if (addBtn) container.insertBefore(newBlock, addBtn);
    else container.appendChild(newBlock);
    pushUndo('insert', { inserted: newBlock });
    setupBlock(newBlock);
    const img = newBlock.querySelector('[data-editable-photo]');
    if (!img) return;
    toast('Compressing dropped photo…');
    try {
      img.src = await compressImage(imageFile);
      toast('Photo inserted.');
    } catch (err) {
      toast('Photo insert failed.');
      newBlock.remove();
    }
  });
